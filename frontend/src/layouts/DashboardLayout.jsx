...
export default function DashboardLayout() {
	...
	useEffect(() => {
		if (!token) return
		notificationService.list(token, logout)
			.then(items => {
				if (Array.isArray(items)) {
					setUnreadCount(items.filter(i => i.status === 'UNREAD').length)
					setRecentAlerts(items.slice(0, 3))
				}
			})
			.catch(() => {})
